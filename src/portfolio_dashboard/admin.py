...
from .db import add_transaction, create_portfolio, fetch_portfolios_admin, import_transactions_for_portfolio, update_portfolio_visibility
from .validators import validate_transactions
from .snapshots import create_snapshot


def admin_login() -> bool:
    """Password-only admin mode for Streamlit."""
...
def render_admin_panel(transactions_df: pd.DataFrame, holdings_df: pd.DataFrame, summary_df: pd.DataFrame) -> None:
    """Render admin CRUD tools."""
    st.subheader("Admin Panel")
    st.caption("Add portfolios, add trades, import CSV trades, hide/show portfolios, and create snapshots.")
    portfolios = fetch_portfolios_admin()
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Add Portfolio", "Add Transaction", "Bulk Import", "Visibility", "Create Snapshot"])
    with tab1:
    ...
    with tab3:
        ...
        st.caption("CSV columns needed: trade_date, ticker, side, quantity, price, fees, notes. portfolio_id is ignored because you select the destination portfolio here.")
        if portfolios.empty:
            st.warning("Create a portfolio first.")
        else:
    ...
    with tab5:
        ...
        if portfolios.empty:
            ...
            if not public_portfolios:
                st.warning("No dashboard holdings available to snapshot.")
            else:
                ...
                if portfolio_row.empty:
                    st.warning("This portfolio is not available in the admin portfolio table.")
                else:
                    ...
                    if st.button("Create snapshot"):
                        create_snapshot(portfolio_uuid, snapshot_date, snapshot_name, total_mv, portfolio_holdings, metrics, notes)
